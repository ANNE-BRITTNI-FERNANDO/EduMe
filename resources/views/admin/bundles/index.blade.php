    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        updateButtons.forEach(button => {
            button.addEventListener('click', async () => {
                ...
                const userConfirmation = confirm(`Are you sure you want to update the bundle status to "${status}"?`);
                ...
                try {
                    ...
                    if (response.ok && result.success) {
                        alert(`Bundle status successfully updated to "${status}"!`);
                    } else {
                        alert('Failed to update status. Please try again.');
                    }
                } catch (error) {
                    // Log the error silently in the console for debugging but do not alert users unnecessarily
